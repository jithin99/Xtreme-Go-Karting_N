  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
            ...
            {rows.map((b) => (
              <div key={b.id} className="card p-4">
                <div className="flex flex-wrap justify-between">
                  <div>
                    ...
                    <div className="text-xs text-zinc-600">
                      {dayjs(b.startsAt).format('DD MMM YYYY, HH:mm')}
                    </div>
                  </div>
                ...
                <ul className="mt-2 text-sm list-disc pl-5">
                  {b.items.map((it, i) => (
                    <li key={i}>
                      {it.qty}× {it.productId} — {it.variantId} {it.gopro ? '• Go-Pro' : ''}
                    </li>
                  ))}
                </ul>
                ...
                {b.charges && b.charges.length > 0 && (
                  <div className="mt-2 text-xs">
                    Extra charges: {b.charges.map((c) => `${c.label} (₹${c.amount})`).join(', ')}
                  </div>
                )}
              </div>
            ))}
        ...
        <a className="text-xs text-zinc-600 block mt-6" href="#home">← Back to site</a>
      </div>
    </div>
  )
}
